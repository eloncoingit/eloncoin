Eloncoin Core version *0.15.1* is now available from:

  <https://download.eloncoin.org/eloncoin-0.15.1/>
...
A number of changes to the way Eloncoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 elons per kB (0.00001 ELC). This is to prepare for dropping the minimum transaction fee to 0.1 elons per kB (0.0001 ELC) in the next release.
